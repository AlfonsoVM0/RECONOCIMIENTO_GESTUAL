function results = boxcox(y,x,lamlo,lamup,model,foptions)
...
if nargin == 6
 if model == 0
  ...
  [lam fval exitf out] = fminbnd(box_lik(0,y,x,model),lamlo,lamup,foptions);
...
foptions = optimset('Display','off');
 %%[lam fval exitf out] = fminbnd('box_lik',lamlo,lamup,foptions,y,x,model);
   box_lik = @(x) box_lik(1,y,x,model);
   [lam fval exitf out] = fminbnd('box_lik',lamlo,lamup,foptions);
niter = out.iterations;
llike = -fval;
